 ...
 (function($){
	...
	$.fn.tablePagination = function(settings) {
		var defaults = {  
			...
			ignoreRows : [],
			divTable: true, /* create by default tablePagination_top div*/
			topNav : false
		};  
      ...
      function createPaginationElements() {
        var htmlBuffer = [];
        htmlBuffer.push("<div id='tablePagination' style=' width:" + defaults.width + "px'>");
        ...
        htmlBuffer.push("<div id='tablePagination_separator'></div>");
        htmlBuffer.push("<div id='tablePagination_group'>");
        htmlBuffer.push("<div id='tablePagination_firstPage'><span></span></div>");
        htmlBuffer.push("<div id='tablePagination_prevPage'><span></span></div>");
        htmlBuffer.push("</div>");
        htmlBuffer.push("<div id='tablePagination_separator'></div>");
        htmlBuffer.push("<div id='tablePagination_group'>");
        htmlBuffer.push("<span id='tablePagination_control'>");
        htmlBuffer.push("Pag");
        ...
        htmlBuffer.push("<div id='tablePagination_separator'></div>");
        htmlBuffer.push("<div id='tablePagination_group'>");
        htmlBuffer.push("<div id='tablePagination_nextPage'><span></span></div>");
        htmlBuffer.push("<div id='tablePagination_lastPage'><span></span></div>");
        htmlBuffer.push("</div>");
        htmlBuffer.push("<div id='tablePagination_separator'></div>");
        htmlBuffer.push("</div>");
        htmlBuffer.push("</div>");
      ...
      if ($(table)[tblLocation]().find(totalPagesId).length == 0) {
        ...
        tDiv.id = 'tablePagination_top';
        hDiv = document.createElement('div');
        hDiv.id = 'tablePagination_head';
		if (defaults.topNav) {
			$(this).before(hDiv);
			$(this).before(createPaginationElements());
			if (defaults.divTable) {
			   $(this).after(tDiv);
		       $(tDiv).prepend($(this));
		    }
		} else {
		    if (defaults.divTable) {
		       ...
		       $(this).parent().before(hDiv);
		       $(this).parent().after(createPaginationElements());
		    }
		}
		if (defaults.divTable) {
		   $(tDiv).css('height', defaults.height);
		   $(tDiv).css('width', defaults.width);
		   table = tDiv;
		} else {
		   table = $(this).parent();
		}
		$(hDiv).css('width', defaults.width);
      }
      else {
        $(table)[tblLocation]().find(currPageId).val(currPageNumber);
      }
